...
/// @param T: The sequence length of the input.
/// @param C: The embedding dimension or channel size of the embeddings.
pub fn encoder_forward_vec(comptime N: usize, output: []LlmFloat, input: []u32, wte: []LlmFloat, wpe: []LlmFloat, B: u32, T: u32, C: u32) void {
    for (0..B) |b| {
        for (0..T) |t| {
            for (0..C / N) |i| {
                const ix: u32 = input[b * T + t];
...
/// @param T: Sequence length, as defined in the forward function.
/// @param C: Embedding dimension, as defined in the forward function.
pub fn encoder_backward_vec(comptime N: u32, dwte: []LlmFloat, dwpe: []LlmFloat, dout: []LlmFloat, inp: []u32, B: u32, T: u32, C: u32) void {
    for (0..B) |b| {
        for (0..T) |t| {
            for (0..C / N) |i| {
...
pub fn layernorm_forward(output: []LlmFloat, mean: []LlmFloat, rstd: []LlmFloat, input: []LlmFloat, weight: []LlmFloat, bias: []LlmFloat, B: u32, T: u32, C: u32) void {
...
/// @param T: Sequence length.
/// @param C: Number of features (embedding dimension in the context of transformers).
pub fn layernorm_forward_vec(comptime N: usize, output: []LlmFloat, mean: []LlmFloat, rstd: []LlmFloat, input: []LlmFloat, weight: []LlmFloat, bias: []LlmFloat, B: u32, T: u32, C: u32) void {
    const eps: LlmFloat = 1e-5;
...
pub fn layernorm_backward(dinp: []LlmFloat, dweight: []LlmFloat, dbias: []LlmFloat, doutput: []LlmFloat, inp: []LlmFloat, weight: []LlmFloat, mean: []LlmFloat, rstd: []LlmFloat, B: u32, T: u32, C: u32) void {
    for (0..B) |b| {
        for (0..T) |t| {
...
/// @param C: Number of input channels or features per time step.
/// @param OC: Number of output channels, corresponding to the number of neurons in a fully connected layer.
pub fn matmul_forward_vec(comptime N: usize, output: []LlmFloat, input: []LlmFloat, weight: []LlmFloat, bias: []LlmFloat, B: usize, T: usize, C: usize, OC: usize) void {
    for (0..B) |b| {
        for (0..T) |t| {
...
pub fn matmul_backward(dinp: []LlmFloat, dweight: []LlmFloat, dbias: []LlmFloat, dout: []LlmFloat, inp: []LlmFloat, weight: []LlmFloat, B: u32, T: u32, C: u32, OC: u32) void {
    for (0..B) |b| {
        for (0..T) |t| {
...
/// @param C: Number of input channels or features.
/// @param OC: Number of output channels.
pub fn matmul_backward_vec(comptime N: usize, dinp: []LlmFloat, dweight: []LlmFloat, dbias: []LlmFloat, dout: []LlmFloat, inp: []LlmFloat, weight: []LlmFloat, B: u32, T: u32, C: u32, OC: usize) void {
    for (0..B) |b| {
        for (0..T) |t| {
            const dout_bt = dout[b * T * OC + t * OC ..];
...
pub fn attention_forward(output: []LlmFloat, preatt: []LlmFloat, att: []LlmFloat, inp: []LlmFloat, B: u32, T: u32, C: u32, NH: u32) void {
    const C3: u32 = C * 3;
...
/// @param C: Dimension of each input token, which is split into parts for query, key, and value.
/// @param NH: Number of attention heads.
pub fn attention_forward_vec(comptime N: usize, output: []LlmFloat, preatt: []LlmFloat, att: []LlmFloat, inp: []LlmFloat, B: usize, T: usize, C: usize, NH: usize) void {
    const C3: usize = C * 3;
...
/// @param NH: Number of attention heads.
pub fn attention_backward(dinp: []LlmFloat, dpreatt: []LlmFloat, datt: []LlmFloat, dout: []LlmFloat, inp: []LlmFloat, att: []LlmFloat, B: u32, T: u32, C: u32, NH: u32) void {
    const C3: u32 = C * 3;
...
/// @param C: Channel size of the input.
/// @param NH: Number of attention heads.
pub fn attention_backward_vec(comptime NChannelsPerHead: usize, dinp: []LlmFloat, dpreatt: []LlmFloat, datt: []LlmFloat, dout: []LlmFloat, inp: []LlmFloat, att: []LlmFloat, B: u32, T: u32, C: u32, NH: u32) void {
    const C3: u32 = C * 3;
    const hs: u32 = @intCast(C / NH);
    const hs_float: LlmFloat = @floatFromInt(hs);
    const scale: LlmFloat = 1.0 / math.sqrt(hs_float);
...
pub fn gelu_backward(dinput: []LlmFloat, input: []LlmFloat, doutput: []LlmFloat, N: u32) void {
    const s = math.sqrt(2.0 / math.pi);
    for (0..N) |i| {
        const x = input[i];
...
fn crossentropy_softmax_backward(dlogits: []LlmFloat, dlosses: []LlmFloat, probs: []LlmFloat, targets: []u32, B: u32, T: u32, V: u32) void {
    for (0..B) |b| {
        for (0..T) |t| {
...
fn gpt2_forward(allocator: std.mem.Allocator, model: *GPT2, inputs: []u32, targets: []u32, B: u32, T: u32) !void {
    ...
    biases.len = 0;
    if ((C % VectorSize == 0) and (C > VectorSize)) {
        layernorm_forward_vec(VectorSize, model.acts.lnf, model.acts.lnf_mean, model.acts.lnf_rstd, residual, model.params.lnfw, model.params.lnfb, B, T, C);
        matmul_forward_vec(VectorSize, model.acts.logits, model.acts.lnf, model.params.wte, biases, B, T, C, V);
    } else {
        ...
        matmul_forward(model.acts.logits, model.acts.lnf, model.params.wte, biases, B, T, C, V);
    }
...
fn gpt2_backward(allocator: std.mem.Allocator, model: *GPT2) !void {
    ...
    if ((C % VectorSize == 0) and (C > VectorSize)) {
        ...
        matmul_backward_vec(VectorSize, model.grads_acts.lnf, model.grads.wte, dbiases, model.grads_acts.logits, model.acts.lnf, model.params.wte, B, T, C, V);
    } else {
        crossentropy_softmax_backward(model.grads_acts.logits, model.grads_acts.losses, model.acts.probs, model.targets, B, T, V);
        matmul_backward(model.grads_acts.lnf, model.grads.wte, dbiases, model.grads_acts.logits, model.acts.lnf, model.params.wte, B, T, C, V);
    }
    ...
    while (l >= 0) {
        ...
        if ((C % VectorSize == 0) and (C > VectorSize)) {
            ...
            matmul_backward_vec(VectorSize, dl_fch_gelu, dl_fcprojw, dl_fcprojb, dl_fcproj, l_fch_gelu, l_fcprojw, B, T, 4 * C, C);
            gelu_backward(dl_fch, l_fch, dl_fch_gelu, B * T * 4 * C);
            matmul_backward_vec(VectorSize, dl_ln2, dl_fcw, dl_fcb, dl_fch, l_ln2, l_fcw, B, T, C, 4 * C);
            layernorm_backward(dl_residual2, dl_ln2w, dl_ln2b, dl_ln2, l_residual2, l_ln2w, l_ln2_mean, l_ln2_rstd, B, T, C);
            residual_backward(dresidual, dl_attproj, dl_residual2, B * T * C);
            matmul_backward_vec(VectorSize, dl_atty, dl_attprojw, dl_attprojb, dl_attproj, l_atty, l_attprojw, B, T, C, C);
            attention_backward_vec(VectorSize, dl_qkv, dl_preatt, dl_att, dl_atty, l_qkv, l_att, B, T, C, NH);

            matmul_backward_vec(VectorSize, dl_ln1, dl_qkvw, dl_qkvb, dl_qkv, l_ln1, l_qkvw, B, T, C, 3 * C);
    ...
    if ((C % VectorSize == 0) and (C > VectorSize)) {
        encoder_backward_vec(VectorSize, model.grads.wte, model.grads.wpe, model.grads_acts.encoded, model.inputs, B, T, C);
    } else {
...
/// @return void: This function returns nothing but can throw errors if memory allocation fails.
fn gpt2_update(allocator: std.mem.Allocator, model: *GPT2, learning_rate: LlmFloat, beta1: LlmFloat, beta2: LlmFloat, epsilon: LlmFloat, weigthed_decay: LlmFloat, t: u32) !void {
    if (!model.init_adam) {
    ...
    for (0..model.num_parameters) |i| {
        ...
        const m_hat: LlmFloat = m / (1.0 - std.math.pow(LlmFloat, beta1, t_float));
        const v_hat: LlmFloat = v / (1.0 - std.math.pow(LlmFloat, beta2, t_float));

        // update
        model.m_memory[i] = m;
        model.v_memory[i] = v;
        const sqrt_v_hat: LlmFloat = std.math.sqrt(v_hat);
        const tmp = learning_rate * (m_hat / (sqrt_v_hat + epsilon) + weigthed_decay * param);
        model.params_memory[i] -= tmp;
    }
}
// DataLoader
fn dataloader_init(allocator: std.mem.Allocator, loader: *DataLoader, filename: []const u8, B: u32, T: u32) !void {
    loader.B = B;
    loader.T = T;
